...
| **SMBv1 off** | The 1980s dialect behind **EternalBlue / WannaCry / NotPetya**: no signing worth the name, no encryption, and a parser the world spent 2017 patching. Nothing built this decade needs it - the clients that do are the ones that should not be on the network. Three doors, each **skipped, not failed, when Windows says it is not there**: the **server dialect** (`Set-SmbServerConfiguration -EnableSMB1Protocol $false`, read back through `Get-SmbServerConfiguration` - the effective config, the `sshd -T` of SMB), the **client redirector** (the `mrxsmb10` driver disabled and dropped from `LanmanWorkstation`'s dependency list - Microsoft's own procedure - so this box never speaks SMB1 outbound either: a downgrade attack needs a willing client) and the **optional feature** (`SMB1Protocol`, removed with `-NoRestart`; a reboot may be needed to finish it, and the step says so). A modern Server image ships all three off/absent, so the CI **plants the server dialect on** before hardening - step 14 provably shuts a door instead of confirming a default - and verify asserts the other two as absent. The audit graded SMBv1 for a long time as "wider than the baseline"; it is the baseline now. |
...
Early but honest: 14 steps, 50 verify checks (eight of them behavioural), a scored
